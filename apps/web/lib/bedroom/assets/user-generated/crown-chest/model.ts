...
export interface CrownChestOptions {
  ...
  scale?: number;
  /** replace the runtime gradient shader with portable glTF vertex colors */
  exportSafeMaterials?: boolean;
}
...
function sampleGradient(t: number, target: THREE.Color) {
  const normalized = THREE.MathUtils.clamp(t, 0, 1);
  const upperIndex = GRADIENT_STOPS.findIndex((stop) => stop.t >= normalized);
  if (upperIndex <= 0) return target.setHex(GRADIENT_STOPS[0].c);
  const upper = GRADIENT_STOPS[upperIndex];
  const lower = GRADIENT_STOPS[upperIndex - 1];
  const amount = THREE.MathUtils.inverseLerp(lower.t, upper.t, normalized);
  return target.setHex(lower.c).lerp(new THREE.Color(upper.c), amount);
}

function applyPortableEnamelVertexColors(root: THREE.Group, enamel: THREE.MeshPhysicalMaterial) {
  root.updateMatrixWorld(true);
  const vertex = new THREE.Vector3();
  const color = new THREE.Color();
  root.traverse((object) => {
    if (!(object instanceof THREE.Mesh) || object.material !== enamel) return;
    const position = object.geometry.getAttribute('position');
    const colors = new Float32Array(position.count * 3);
    for (let index = 0; index < position.count; index += 1) {
      vertex.fromBufferAttribute(position, index).applyMatrix4(object.matrixWorld);
      sampleGradient((vertex.y - CHEST_MIN_Y) / (CHEST_MAX_Y - CHEST_MIN_Y), color);
      color.toArray(colors, index * 3);
    }
    object.geometry.setAttribute('color', new THREE.BufferAttribute(colors, 3));
  });

  enamel.name = 'portable-enamel-vertex-gradient';
  enamel.onBeforeCompile = THREE.Material.prototype.onBeforeCompile;
  enamel.customProgramCacheKey = THREE.Material.prototype.customProgramCacheKey;
  enamel.vertexColors = true;
  enamel.needsUpdate = true;
}
...
export function createSourceModel(options: CrownChestOptions = {}): THREE.Group {
  ...
  const lidBrackets = new THREE.Group();
  lidBrackets.name = 'lid-corner-brackets';
  lidHinge.add(lidBrackets);
  const bodyBrackets = new THREE.Group();
  bodyBrackets.name = 'body-corner-brackets';
  root.add(bodyBrackets);
  const corners: { pos: [number, number, number]; sx: number; sy: number; sz: number; lid: boolean }[] = [
    // top row (on the lid)
    { pos: [0.5, 0.39, 0.41], sx: 1, sy: 1, sz: 1, lid: true },
    { pos: [-0.5, 0.39, 0.41], sx: -1, sy: 1, sz: 1, lid: true },
    { pos: [0.5, 0.39, -0.41], sx: 1, sy: 1, sz: -1, lid: true },
    { pos: [-0.5, 0.39, -0.41], sx: -1, sy: 1, sz: -1, lid: true },
    // bottom row (on the body)
    { pos: [0.5, -0.46, 0.41], sx: 1, sy: -1, sz: 1, lid: false },
    { pos: [-0.5, -0.46, 0.41], sx: -1, sy: -1, sz: 1, lid: false },
    { pos: [0.5, -0.46, -0.41], sx: 1, sy: -1, sz: -1, lid: false },
    { pos: [-0.5, -0.46, -0.41], sx: -1, sy: -1, sz: -1, lid: false },
  ];
  for (const c of corners) {
    ...
    if (c.lid) {
      b.position.set(
        c.pos[0] - lidHinge.position.x,
        c.pos[1] - lidHinge.position.y,
        c.pos[2] - lidHinge.position.z,
      );
    } else {
      b.position.set(...c.pos);
    }
    b.scale.set(c.sx, c.sy, c.sz);
    (c.lid ? lidBrackets : bodyBrackets).add(b);
  }
  nodes.lidBrackets = lidBrackets;
  nodes.bodyBrackets = bodyBrackets;
  ...
  root.userData.sculptRuntime = {
    ...
    destructionGroups: ['lid', 'body', 'lid-corner-brackets', 'body-corner-brackets', 'front-latch', 'side-handle', 'crown-emblem'],
  };

  if (options.exportSafeMaterials) applyPortableEnamelVertexColors(root, enamel);
